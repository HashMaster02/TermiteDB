...
#define HM_ALIGN                                                               \
    16 /* keeps every key and value suitably aligned for any type */
...
static inline uint64_t hm_hash_bytes(const void *key, size_t key_size) {
    ...
    return h ^
           (h >> 32); /* fold high bits down; the table masks the low bits */
}
...
static inline size_t hm__find(const hashmap *m, const void *key, uint64_t h) {
    ...
    for (i = (size_t)h & mask; m->hashes[i] != 0; i = (i + 1) & mask) {
        if (m->hashes[i] == h && m->eq(hm__key(m, i), key, m->key_size))
            return i;
    }
    return SIZE_MAX;
}
...
static inline int hm__resize(hashmap *m, size_t new_cap) {
    ...
    if (new_cap > SIZE_MAX / m->stride || new_cap > SIZE_MAX / sizeof *hashes)
        return -1;
...
static inline hashmap *hm_new_ex(size_t key_size, size_t val_size,
                                 hm_hash_fn hash, hm_eq_fn eq) {
    hashmap *m;
    if (key_size == 0 || key_size > SIZE_MAX / 4 || val_size > SIZE_MAX / 4)
        return NULL;
    m = calloc(1, sizeof *m);
    if (!m)
        return NULL;
...
/* Number of entries. */
static inline size_t hm_len(const hashmap *m) { return m->len; }
...
static inline int hm_put(hashmap *m, const void *key, const void *val) {
    ...
    if (i != SIZE_MAX) {
        if (m->val_size)
            memcpy(hm__val(m, i), val, m->val_size);
        return 0;
    }
    if (hm__grow_if_needed(m) != 0)
        return -1;
...
static inline void hm_clear(hashmap *m) {
    if (m->cap)
        memset(m->hashes, 0, m->cap * sizeof *m->hashes);
    m->len = 0;
}
...
/* Iteration: `hm_iter it = hm_begin(m); while (hm_next(&it)) use(it.key,
 * it.val);` Entries are visited in unspecified order. */
